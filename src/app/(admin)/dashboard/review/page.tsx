  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col gap-8 px-6 py-10">
      <div className="flex items-end justify-between gap-4">
        <div>
          <p className="text-sm uppercase tracking-[0.25em] text-muted-foreground">Supervisor</p>
            ...
            >
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex-1 space-y-1">
                  <p className="font-semibold">{job.properties?.name ?? "Unknown property"}</p>
                  <p className="text-sm text-muted-foreground">
                    Cleaner: {job.cleaners?.full_name ?? "Unassigned"}
                  </p>
                  <p className="text-sm text-muted-foreground">Due: {formatDate(job.due_at)}</p>
